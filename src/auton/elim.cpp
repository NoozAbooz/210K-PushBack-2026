#include "abstractGlobals.hpp"
#include "deviceGlobals.hpp"
#include "libKW/drivetrain/chassis.hpp"
#include "libKW/drivetrain/movements.hpp"
#include "main.h"
#include "libKW/api.hpp"
#include "pros/motors.h"

void right_elim() {
      pros::Task([] {
        intakeMacro("L1");
        wingPiston.set_value(true);
        wing2Piston.set_value(true);
        pros::delay(390);
        loaderPiston.set_value(true);
       // blockerPiston.set_value(true);
        pros::delay(300);
        loaderPiston.set_value(false);
        pros::delay(1100);
        loaderPiston.set_value(true);
    });
    kw::moveToPoint(6, 21.67, 1000, true, 127, false); // Group of 3
    kw::boomerang(33, 39.3, 90, 0.20, 1500, true, 75); // Under Bar
    pros::delay(350);
    kw::moveToPoint(18, 21.67, 1000, false, 127, false); // Backing Out
    kw::boomerang(39, 6, 180, 0.5, 2000, false, 75); // moving to long goal
    // pros::Task([]{
    //     pros::delay(450);
    //         intakeMacro("R1");
    //         pros::delay(1500);
    //         intakeMacro("L1");

    // });
    // kw::moveToPoint(38.4, 19, 1000, false);  //at long goal
    // kw::move_raw(-7000, -7000);
    // loaderPiston.set_value(true);
    // pros::delay(700);
    kw::moveToPoint(39, -14, 700, true, 70, false); // drive to loader
    kw::move_raw(6000, 6000);
    pros::delay(250);
         pros::Task([]{
        pros::delay(650);
            intakeMacro("R1");
            pros::delay(650);
            intakeMacro("L2");
            pros::delay(350);
            intakeMacro("stop");
    });
    kw::moveToPoint(39, 19, 1000, false);  //at long goal
    kw::move_raw(-7000, -7000);
    pros::delay(800);
    kw::turnToAngle(315, 1000);
    loaderPiston.set_value(false);
    kw::boomerang(.3, 38.5, 315, 0.3, 2000 , true, 80);  // mid goal
    intakeMacro("L2");
    intakeMain.move_voltage(-5000);
    intakeSub.move_voltage(-5000);
    kw::driveTo(1, 1000, 80);
    pros::delay(1500);
    kw::moveToPoint(29, 12, 1000, false); // move to Wing
    wing2Piston.set_value(false);
    kw::turnToAngle(0, 700);
    kw::moveToPoint(29, 33, 8000, true);
    wingStatus = true;
    wing2status = false;
    // kw::move_raw(1000, 1000); 
    // pros::delay(300);
    // pros::Task([]{
    //     pros::delay(400);
    //     intakeMacro("stop");
    //     pros::delay(1300);
    //     intakeMacro("L2");
    //     intakeMain.move_voltage(-7000);
    //     pros::delay(4000);
    //     intakeMacro("stop");
    //     intake.move_voltage(0);
    // });
    // kw::moveToPoint(38, 1, 1000, false, 127, false);
    // loaderPiston.set_value(false);
    // kw::turnToAngle(315, 700);
    // kw::moveToPoint(2, 37.5, 2000); // mid goal
    // pros::delay(2000);
    // kw::moveToPoint(29, 15, 1000, false); // move to Wing
    // kw::turnToAngle(0, 700);
    // kw::moveToPoint(29, 40, 8000, true, 100); // WIng

    // //kw::boomerang(-1.5, 34 , 315, 0.3, 2000 , false, 127);  // mid goal
}

void left_elim() {
    pros::Task([] {
        intakeMacro("L1");
        pros::delay(390);
        loaderPiston.set_value(true);
       // blockerPiston.set_value(true);
        pros::delay(300);
        loaderPiston.set_value(false);
        pros::delay(1100);
        loaderPiston.set_value(true);
    });
    kw::moveToPoint(-6, 21.67, 1000, true, 127, false); // Group of 3
    kw::boomerang(-27, 39.0, 270, 0.3, 2500, true, 60); // Under Bar
    pros::delay(200);
    kw::moveToPoint(-13, 21.67, 1000, false, 127, false); // Backing Out
    loaderPiston.set_value(false);
    kw::boomerang(-38, 9, 180, 0.5, 2000, false, 70); // moving to long goal
    pros::Task([]{
        pros::delay(450);
            intakeMacro("R1");
            pros::delay(1500);
            intakeMacro("L1");

    });
    kw::moveToPoint(-38.4, 19, 1000, false);  //at long goal
    kw::move_raw(-7000, -7000);
    loaderPiston.set_value(true);
    pros::delay(700);
    kw::moveToPoint(-38.5, -14, 1000, true, 70, false); // drive to loader
    kw::move_raw(6000, 6000);
    pros::delay(300);
    pros::Task([]{
        pros::delay(2000);
        intakeMacro("R2");
        pros::delay(1500);
        intakeMacro("L1");
        intake.move_voltage(0);
    });
    kw::boomerang(-1.5, 34 , 215, 0.3, 2000 , false, 127);  // mid goal
    kw::move_raw(-4000, -4000);
    pros::delay(800);
    kw::moveToPoint(-25.4, 17, 1000); // move to Wing
    kw::turnToAngle(180, 700);
    kw::moveToPoint(-27, 37, 1000, false); // WIng

wing2status = false;
wingStatus = false;

}

void R_6and3() {
    pros::Task([] {
        intakeMacro("L1");
        pros::delay(390);
        loaderPiston.set_value(true);
       // blockerPiston.set_value(true);
        pros::delay(300);
        loaderPiston.set_value(false);
        pros::delay(1100);
        loaderPiston.set_value(true);
    });
    kw::moveToPoint(6, 21.67, 1000, true, 127, false); // Group of 3
  kw::boomerang(32, 39.3, 90, 0.20, 1500, true, 75); // Under Bar
    pros::delay(250);
    kw::moveToPoint(18, 21.67, 1000, false, 127, false); // Backing Out
    kw::boomerang(38, 10, 180, 0.5, 2000, false, 90); // moving to long goal
     pros::Task([]{
        pros::delay(750);
            intakeMacro("R1");
            pros::delay(1000);
            intakeMacro("stop");
    });
    kw::moveToPoint(38.4, 19, 1000, false);  //at long goal
    kw::move_raw(-7000, -7000);
    pros::delay(1000);
     kw::moveToPoint(38, -15, 700, true, 70, false); // drive to loader
    kw::move_raw(6000, 6000);
    pros::delay(250);
    kw::driveTo(-10, 1000);
     kw::turnToAngle(315, 1000);
    loaderPiston.set_value(false);
    kw::boomerang(1.8, 38.5, 315, 0.3, 2000 , true, 80);  // mid goal
    intakeMacro("L2");
    intakeMain.move_voltage(-5000);
    intakeSub.move_voltage(-5000);
    kw::driveTo(1, 1000, 80);
}

void L_5and4() {
      pros::Task([] {
        intakeMacro("L1");
        pros::delay(390);
        loaderPiston.set_value(true);
       // blockerPiston.set_value(true);
        pros::delay(300);
        loaderPiston.set_value(false);
        pros::delay(850);
        loaderPiston.set_value(true);
    });
    kw::moveToPoint(-6, 21.67, 1000, true, 127, false); // Group of 3
    kw::boomerang(-33, 42, 280, 0.10, 1800, true, 75); // Under Bar
    pros::delay(350);
    kw::moveToPoint(-20, 21.67, 1000, false, 127, false); // Backing Out
    kw::boomerang(-39, 6, 180, 0.5, 2000, false, 90); // moving to long goal
    // pros::Task([]{
    //     pros::delay(450);
    //         intakeMacro("R1");
    //         pros::delay(1500);
    //         intakeMacro("L1");

    // });
    // kw::moveToPoint(-38.4, 19, 1000, false);  //at long goal
    // kw::move_raw(-7000, -7000);
    // loaderPiston.set_value(true);
    // pros::delay(700);
    kw::moveToPoint(-39, -14, 700, true, 70, false); // drive to loader
    kw::move_raw(6000, 6000);
    pros::delay(350);
         pros::Task([]{
        pros::delay(700);
            intakeMacro("R1");
            pros::delay(700);
            intakeMacro("L2");
            pros::delay(350);
            intakeMacro("L1");
    });
    kw::moveToPoint(-39, 20, 1000, false);  //at long goal
    kw::move_raw(-7000, -7000);
    pros::delay(900);
    kw::driveTo(12, 1000);
    	pros::Task([] {
		pros::delay(800);
        intakeMacro("L2");
        pros::delay(200);
		intakeMacro("R2");
	// intakeCounterRoller.move_voltage(-1300);
		intakeMain.move_voltage(5400);
		intakeSub.move_voltage(5400);
		intakeCounterRoller.move_voltage(-2500);
	});
    kw::boomerang(3.2, 41, 225, 0.4, 2000 , false, 80);  // mid goal
    pros::delay(1000);
    kw::moveToPoint(-26.3, 16.94, 1000, true, 127); //align for wing
    kw::turnToAngle(180, 700);
    intakeMacro("stop");
    //intake_velocity.set_target(0);
    loaderPiston.set_value(false);
    kw::moveToPoint(-25.803, 34.84, 8000, false, 80); //wing
    // kw::turnToAngle(180, 700);
    //kw::driveTo(-1, 1000, 70);
   
    kw::boomerang(-26, 31, 226, 0.3, 1000, true, 100); //turn into post
    kw::stop_chassis(pros::E_MOTOR_BRAKE_HOLD);

    wing2status = false;
wingStatus = false;

};

void left_7() {
pros::Task([] {
        intakeMacro("L1");
        pros::delay(390);
        loaderPiston.set_value(true);
    });
  //  kw::moveToPoint(-6, 21.67, 1000, true, 127, false); // Loader #1
  kw::boomerang(-6, 23, 233, 0.1, 1500, true, 110);
    //kw::turnToAngle(233, 700);
   // kw::moveToPoint(-35.59, 1.93, 2000, true, 127); 
    //kw::turnToAngle(180, 700);
   // kw::moveToPoint(-36.95, -10.5, 700, true, 70); // Long Goal #1
    kw::boomerang(-32.05, -7.5, 180, 0.4, 1500, true, 100);
    kw::moveToPoint(-36.65, -10.5, 700, true, 70); // Long Goal #1
    //kw::driveTo(3, 1000, 70);
    kw::move_raw(7000, 7000);
    pros::delay(450);
    pros::Task([] {
		pros::delay(300);
		stopIntake();
		pros::delay(400);
		intakeMacro("R1");
        loaderPiston.set_value(false);
	});
    kw::moveToPoint(-35.439, 24.45, 1000, false, 127);
    kw::move_raw(-5000, -5000);
    pros::delay(1000);
    kw::moveToPoint(-22.2, 16.94, 1000, true, 127, false); //align for wing
    // kw::turnToAngle(180, 700);
    intakeMacro("stop");
    //intake_velocity.set_target(0);
    loaderPiston.set_value(false);
    kw::boomerang(-24.703, 34.84, 180, 0.3, 1000, false);
    // kw::turnToAngle(180, 700);
    //kw::driveTo(-1, 1000, 70);
   
    kw::boomerang(-25, 31, 226, 0.3, 1000, true, 100); //turn into post
    kw::stop_chassis(pros::E_MOTOR_BRAKE_HOLD);

    wing2status = false;
wingStatus = false;

    
   // **old Stuff below**
   
    //  toggleColourSort = true;
    //  intakeMacro("L1");
    //  pros::Task([] {
    //     pros::delay(750);
    //     loaderPiston.set_value(true);
    // });
    // kw::driveTo(28.5, 2000, 60);
    // kw::turnToAngle(236, 1000);
    // kw::moveToPoint(-30, 17, 2000);
    // kw::turnToAngle(200, 1000);    
    // kw::moveToPoint(-34.37, 2.7, 1000, true, 70);
    // kw::move_raw(3000, 3000);
    // pros::delay(70);
    // //pros::delay(50);
    // //kw::driveTo(-33, 1000);
    // kw::moveToPoint(-25.2, 33.3, 1000, false);
    // kw::move_raw(-10000, -10000);
    // //stopIntake();
    // pros::delay(200);
    // //toggleColourSort = true;
    // intakeMacro("R1");
    // loaderPiston.set_value(false);
    // pros::delay(2300);
    // stopIntake();
    // kw::moveToPoint(-20.7, 10, 1000);
    // kw::turnToAngle(204, 1000);
    // kw::moveToPoint(-8.7, 48, 8000, false, 60);
    //     kw::turnToAngle(204, 1000);
    // //kw::driveTo(-7, 1000);

}
void right_4() {
    pros::Task([] {
        intakeMacro("L1");
        pros::delay(390);
        loaderPiston.set_value(true);
        pros::delay(3000);
        loaderPiston.set_value(false);
    });
    kw::moveToPoint(6, 21.67, 1000, true, 127, false); // Loader #1
   // kw::turnToAngle(270, 700);
   pros::Task([]{
        pros::delay(1950);
            intakeMacro("R1");
            pros::delay(1500);
            intakeMacro("stop");

    });
    kw::boomerang(38.4, 20, 180, 0.25, 1800 , false, 127, false);  //at long goal
    kw::turnToAngle(180, 700);
    kw::move_raw(-8000, -8000);
   // kw::moveToPoint(38.4, 23, 1000, false);  //at long goal
    pros::delay(700);
    kw::boomerang(50, 10, 180, 0.4, 2000 , true, 127, false);  //at long goal
    kw::moveToPoint(48, 41, 1000, false);  //at long goal
//     kw::moveToPoint(30, 21, 1000, false, 127, false);
//     //kw::swing(180, 1000, false, 127);
//     kw::move_raw(-12000, 3000);
//     pros::delay(1000);
//     kw::turnToAngle(180, 700);
//     kw::move_raw(-5000, -5000);
}

void right_7() {
pros::Task([] {
        intakeMacro("L1");
        pros::delay(390);
        loaderPiston.set_value(true);
    });
    kw::moveToPoint(6, 21.67, 1000, true, 127, false); // Loader #1
    kw::turnToAngle(127, 700);
    kw::moveToPoint(35.59, 1.93, 2000, true, 127); 
    kw::turnToAngle(180, 700);
    kw::moveToPoint(35.95, -11.5, 700, true, 70); // Long Goal #1
    //kw::driveTo(3, 1000, 70);
    kw::move_raw(7000, 7000);
    pros::delay(180);
    pros::Task([] {
		pros::delay(300);
		stopIntake();
		pros::delay(400);
		intakeMacro("R1");
        loaderPiston.set_value(false);
	});
    kw::moveToPoint(35.89, 24.45, 1000, false, 127);
    kw::move_raw(-5000, -5000);
    pros::delay(1600);
    kw::moveToPoint(47, 12.94, 1000, true, 127);
    kw::turnToAngle(180, 700);
    //intake_velocity.set_target(0);
    kw::moveToPoint(48, 32, 8000, false, 100);
    kw::boomerang(46, 32, 226, 0.3, 1000, true, 100);
    kw::stop_chassis(pros::E_MOTOR_BRAKE_HOLD);
    wing2status = false;
wingStatus = false;

    //old stuff below

//     toggleColourSort = false;
//      intakeMacro("L1");
//     kw::driveTo(28.5, 2000, 60);
//     kw::turnToAngle(124, 1000);
//     kw::moveToPoint(30, 17, 2000);
//     loaderPiston.set_value(true);
//     kw::turnToAngle(160, 1000);    
//     kw::moveToPoint(34.87, 2.7, 1500, true, 70);
//     kw::move_raw(7000, 7000);
//     pros::delay(50);
//     //kw::driveTo(-33, 1000);
//     kw::moveToPoint(25.0, 33.3, 1000, false);
//     kw::move_raw(-8000, -8000);
//     //stopIntake();
//     pros::delay(200);
//     toggleColourSort = true;
//     intakeMacro("R1");
//     loaderPiston.set_value(false);
//     pros::delay(2200);
//     stopIntake();
//     kw::moveToPoint(+38.1, 23.53, 1000);
//     kw::turnToAngle(164, 1000);
//     kw::moveToPoint(+30.0, 51, 8000, false, 70);
//     kw::driveTo(-7, 1000);
//     kw::turnToAngle(164, 1000);
}